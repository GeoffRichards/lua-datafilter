...
LUA_CPATH:=$(shell pkg-config lua$(LUAVERSION) --define-variable=prefix=$(PREFIX) \
...
CFLAGS := -std=c99 -pedantic -Wall -W -Wshadow -Wpointer-arith \
          ...
          $(shell pkg-config --cflags lua$(LUAVERSION)) \
          -DVERSION=\"$(VERSION)\"
LDFLAGS := $(shell pkg-config --libs lua$(LUAVERSION))
...
test: all runtests.lua
	$(VALGRIND) lua$(LUAVERSION) runtests.lua
