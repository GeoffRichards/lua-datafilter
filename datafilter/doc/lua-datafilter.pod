...
Encode binary data as text using the Base64 given in S<RFC 4648>.
See L<lua-datafilter-base64(3)> for options.
...
Returns a 16 byte message digest using the algorithm from C<RFC 1321>.

=item sha1

Returns a 20 byte message digest using the algorithm from C<RFC 3174>.
