...
function testcase:test_aaa_md5 ()
    for i = 1, #aaa_md5 do
        local expected = aaa_md5[i]
        local input = ("a"):rep(i)
        local got = Filter.md5(input)
        is(16, got:len())
        is(expected, bytes_to_hex(got),
...
-- Data borrowed from 't/md5-aaa.t' in the Perl module Digest::MD5.  The
-- input string to get each digest should be the ASCII letter 'a' repeated
-- the same number of times as the index into the array.
aaa_md5 = {
